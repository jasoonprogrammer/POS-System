...
def strHour(col):
    ...
    col[0] = f'{d.strftime("%I:%M %p")} - {d2.strftime("%I:%M %p")}'
    return col
        ...
        if len(results) > 0:
            ...
            if pages > 1:
                ...
                for page in range(pages):
                    ax[page].set_xlim(left = 0, right = max(df['Total Revenue']))
                    ax[page].barh(labels[page * n_count:(page + 1) * n_count][::-1], df["Total Revenue"][page * n_count:(page + 1) * n_count][::-1])
            else:
                fig, ax = plt.subplots(figsize=(15, 10))

                ax.set_xlim(left = 0, right = max(df['Total Revenue']))
                ax.barh(labels[::-1], df["Total Revenue"][::-1])
    ...
    def generateProductReportGroup(self, date, writer, format1):
        ...
        if len(results) > 0:
            ...
            if pages > 1:
                for page in range(pages):
                    ax[page].set_xlim(left = 0, right = max(df['Total Revenue']))
                    ax[page].set_ylim(bottom = -.5, top = 7.5)
